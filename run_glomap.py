...
def main():
    ...
    _seen = set()
    all_stages = []
    for s in (GloMapPipeline.STAGES_STANDARD + GloMapPipeline.STAGES_HLOC
              + GloMapPipeline.STAGES_HLOC_MATCHING):
        if s not in _seen:
            all_stages.append(s)
            _seen.add(s)
...
hloc Mode Examples:
  ...
  python run_glomap.py --input_images ./images --output ./output --colmap_config defaultColMap.ini --use_hloc

  # Dense SIFT for Gaussian splatting
  python run_glomap.py --input_images ./images --output ./output --colmap_config defaultColMap.ini \\
